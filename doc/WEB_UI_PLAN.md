...
| P3-08 | ⏯️ DEFERRED | `AddressingModeArrow.svelte` — on-hover SVG overlay drawing an arrow from operand byte in memory grid → effective address cell | Deferred to Phase 4 (lands with P4-03 Ramses and P4-04 Cesar). Ramses/Cesar-specific; co-locating with the first consumer avoids premature generalization. `MemoryGrid` already exposes `hoveredAddr`/`effAddr` hooks so the arrow can overlay as a sibling component |

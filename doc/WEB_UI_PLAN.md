...
| P3-06 | ⏯️ DEFERRED | `SourceView.svelte` — show `.ram`/`.ces` source with PC-aligned highlight when `addrToLine` map is present | Deferred pending assembler metadata. The ported TS assemblers currently return byte arrays only; they need to emit `{ bytes, addrToLine, labels, instrAddrs }` first. Re-open when P5-01 extends the assembler output (the sample-program bundling naturally motivates this change) |
...
| P3-10 | ⏯️ DEFERRED | `DisplayPanel.svelte` — 36-byte ASCII strip rendered as a fixed-width "screen" for Cesar (0xFFDC..0xFFFF) | Deferred to Phase 4 (lands with P4-04 Cesar). Cesar-only |
| P3-11 | ⏯️ DEFERRED | `KeyboardInput.svelte` — input box that writes a byte to 0xFFDA on submit | Deferred to Phase 4 (lands with P4-04 Cesar). Cesar-only |
| P3-12 | ⏯️ DEFERRED | `StackPanel.svelte` — upward-growing column of words starting at R6; appears only when R6 has been written or JSR executed | Deferred to Phase 4 (lands with P4-04 Cesar). Cesar-only |

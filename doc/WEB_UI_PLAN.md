...
|-------|---------|------|-------|
| P1-01 | ❌ OPEN | Scaffold `web/` with pnpm, Astro (Svelte integration), TypeScript strict, Vitest, ESLint+Prettier, `@astrojs/cloudflare` adapter (static output) | Commit `web/package.json`, `web/astro.config.mjs`, `web/tsconfig.json`. Add root `.gitignore` entries for `web/node_modules`, `web/dist`, `web/.astro` |
| P1-02 | ❌ OPEN | Port `Memory.fs` → [web/src/core/memory.ts](../web/src/core/memory.ts) | Include `readCount`/`writeCount`, big-endian word helpers, `load(bytes, address)`. Reference: [fs/ArchSims.Core/Memory.fs](../fs/ArchSims.Core/Memory.fs) |
